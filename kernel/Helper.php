...
if (!function_exists("css_async")) {
    ...
    function css_async(array $resource, bool $cdn = true): string
    {
        ...
        $cdnSupport = $cdn ? ' class="cdn-support"' : '';
        foreach ($resource as $item) {
            $href = htmlspecialchars($item . '?v=' . APP_VERSION . $debugRandom, ENT_QUOTES, 'UTF-8');
            $res .= '<link rel="stylesheet" href="' . $href . '" media="print" onload="this.media=\'all\'"' . $cdnSupport . '>';
            $res .= '<noscript><link rel="stylesheet" href="' . $href . '"' . $cdnSupport . '></noscript>';
        }
        return $res;
    }
}
