...
class Shop implements \App\Service\Shop
{
    ...
    public function getCategory(?UserGroup $group): array
    {
        ...
        $cache = null;
        $cacheKey = '';
        // 主站首页分类树：短时文件缓存，降低首页 TTFB（分站/商家上下文不参与缓存）
        if (!$bus) {
            $cacheKey = 'shop_category_master_v1_' . md5(
                (string)($group?->id ?? 0) . "\0"
                . (string)Config::get("substation_display") . "\0"
                . (string)Config::get("substation_display_list") . "\0"
                . (string)Config::get("commodity_recommend") . "\0"
                . (string)Config::get("commodity_name")
            );
            $cache = new Cache(BASE_PATH . '/runtime/cache/shop', Cache::OPTIONS_SERIALIZE);
        ...
        if (!$bus && $cache !== null && $cacheKey !== '') {
            $cache->set($cacheKey, ['exp' => time() + 60, 'data' => $array]);
        }
