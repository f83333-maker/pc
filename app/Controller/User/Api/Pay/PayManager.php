    ...
    public function get(): Response
    {
        ...
        foreach ($data['list'] as &$item) {
            if (!$item['pid'] && isset($item['config'])) {
                $pluginName = $item['config']['plugin'] ?? '';
                if (!array_key_exists($pluginName, $pluginCache)) {
                    $pluginCache[$pluginName] = Plugin::instance()->getPlugin($pluginName, $env);
                }
                $item['plugin'] = $pluginCache[$pluginName];
