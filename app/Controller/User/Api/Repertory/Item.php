...
class Item extends Base
{
    ...
    public function get(): Response
    {
        ...
        foreach ($data['list'] as &$dat) {
            $pname = $dat['plugin'] ?? '';
            if (!array_key_exists($pname, $pluginCache)) {
                $pluginCache[$pname] = Plugin::instance()->getPlugin($pname, $env);
            }
            $plugin = $pluginCache[$pname];
            if ($plugin) {
            ...
            foreach ($dat['sku'] as &$sku) {
                $rid = (int)($sku['id'] ?? 0);
                $sku['stock'] = ($rid > 0 && array_key_exists($rid, $stockMap))
                    ? $stockMap[$rid]
                    : "异常";
            }
            unset($sku);
