    ...
    public function get(): Response
    {
        ...
        foreach ($data['list'] as $item) {
            foreach ($item['sku'] ?? [] as $sku) {
                if (!empty($sku['repertory_item_sku_id'])) {
                    $skuIds[] = (int)$sku['repertory_item_sku_id'];
                }
            }
        }
        $skuIds = array_values(array_unique($skuIds));
        $stockMap = [];
        foreach ($skuIds as $rid) {
            try {
                $stockMap[$rid] = $this->ship->stock($rid);
            } catch (\Throwable $e) {
                $stockMap[$rid] = "异常";
            }
        }
        ...
        foreach ($data['list'] as $index => $item) {
            foreach ($item['sku'] ?? [] as $key => $sku) {
                $rid = (int)($sku['repertory_item_sku_id'] ?? 0);
                $data['list'][$index]["sku"][$key]['stock'] = ($rid > 0 && array_key_exists($rid, $stockMap))
                    ? $stockMap[$rid]
                    : "异常";
